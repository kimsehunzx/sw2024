...
app.get('/people', function(req, res) {
    var message = "LMAO I hacked you";
    //render ejs page.
    if(req.query.no !== 0) {
        console.log(req.query.no);
         var idx = peopleList.findIndex(function(curPeople) {
            return parseInt(curPeople.no) === parseInt(req.query.no);
        });
        var targetPeople = null;
        if(idx!== -1) {peopleList[idx].name = req.query.name;
                        peopleList[idx].age = req.query.age;
                        peopleList[idx].job = req.query.job;}
        req.app.render('people', {message, peopleList}, function(err, html) {
            res.end(html);
        });
    }
});
